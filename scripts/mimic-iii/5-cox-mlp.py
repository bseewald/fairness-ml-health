...
#     net = make_net(train, dropout=0, num_nodes=[64, 64])
#     model = CoxCC(net, tt.optim.Adam(weight_decay=0.4))
...
def main():
    ...
    surv, model, log = fit_and_predict(CoxCC, train, val, test,
                                       lr=0.01, batch=128, dropout=0, epoch=512, weight_decay=0.01,
                                       num_nodes=[64, 64], shrink=0.01, device=device)
    # Evaluate
    cindex, bscore, bll = evaluate(train, surv)

    # Best Parameters
    _file.write("Best Parameters: " + str() + "\n")

    # Scores
    _file.write("C-Index: " + str() + "\n")
